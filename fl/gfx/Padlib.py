...
class Shadow:
    def __init__(self,size,Pos,occluder,color,lightopacity):
        global mapd, lightRect, lightBody, lightcolor
        self.radius = size
        self.lightPos = list(Pos)
        mapd = occluder
        lightRect = None    
        self.mask = pygame.Surface((self.radius*2,self.radius*2))
        self.mask.set_colorkey((0,0,0))
        self.mask.set_alpha(lightopacity)
        lightRect = self.mask.get_rect()
        lightRect.center = self.lightPos
        lightcolor = color
    ...
    def getPolygon(self,x,y,box):
        ...
        self.lightPos = [self.radius,self.radius]
        if x >= l and x <= r:
            if y >= b: # directly under
                tp1 = self.tracePoint(x,y,l,b,L)
                tp2 = self.tracePoint(x,y,r,b,L)
                return ((box.bottomleft,tp1,[self.lightPos[0]-L,self.lightPos[1]-L],[self.lightPos[0]+L,self.lightPos[1]-L],tp2,box.bottomright))
            else:   # directly above
                tp1 = self.tracePoint(x,y,l,t,L)
                tp2 = self.tracePoint(x,y,r,t,L)
                return ((box.topleft,tp1,[self.lightPos[0]-L,self.lightPos[1]+L],[self.lightPos[0]+L,self.lightPos[1]+L],tp2,box.topright))
        elif y >= t and y <= b:
            if x <= l: # directly to the left
                tp1 = self.tracePoint(x,y,l,b,L)
                tp2 = self.tracePoint(x,y,l,t,L)
                return ((box.bottomleft,tp1,[self.lightPos[0]+L,self.lightPos[1]+L],[self.lightPos[0]+L,self.lightPos[1]-L],tp2,box.topleft))
            else:   # directly to the right
                tp1 = self.tracePoint(x,y,r,b,L)
                tp2 = self.tracePoint(x,y,r,t,L)
                return ((box.bottomright,tp1,[self.lightPos[0]-L,self.lightPos[1]+L],[self.lightPos[0]-L,self.lightPos[1]-L],tp2,box.topright))
        if y <= t:
            if x <= l: # upper left
    ...
    def change_position(self,pos):
        self.lightPos[0] = pos[0]
        self.lightPos[1] = pos[1]
        lightRect.center = list(self.lightPos)
    def DrawMask(self):
        nrects = []
        for r in mapd:
        ...
        pygame.draw.circle(self.mask, lightcolor, (self.radius,self.radius), self.radius, 0)
        for r in nrects:
            p = self.getPolygon(self.radius,self.radius,r)
            if p:
                pygame.draw.polygon(self.mask, (0,0,0), p, 0)
                
    def draw(self,screen):
        self.DrawMask()
        screen.blit(self.mask, lightRect.topleft)
#---ROUNDED RECTANGLE---
